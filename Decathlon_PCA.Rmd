...
TO DO
Introduce a categorical variable e.g. Day 1/day 2
Add extended plotting e.g. colouring
Try out plot.PCA
Subset the data? What are the top 10 Javelines? Now what is the mean/max?
...
hist(decathlon$Javeline)
```
Question: What do these graphs tell you? 
The first graph shows us the distance each competitor threw the javelin. 
The second graph shows the number of competitors who threw the javelin in groups of 5 metres.  
...
```{r}
print ("Average javelin throw")
mean(decathlon$Javeline)

print("Maximum javelin throw")
...
```{r}
plot(sort(decathlon$Javeline), main = "Javeline distance")
...
abline(reg, col = "blue")
#TO DO why does this not work for 100m vs high jump?
```
...
```{r}
plot(decathlon$Discus, decathlon$Shot.put)
```

```{r}
cor(decathlon$Discus, decathlon$Shot.put)
```
...
```{r}
# plot the scaled data for javelin
...
Then plot the PCA of individuals coloured by the competition using fviz_pca_ind()
Hint: use repel=TRUE to read the names more easily


```{r}
res.pca <- PCA(dec_scaled, quali.sup = "Competition")

fviz_pca_ind(res.pca, habillage = "Competition", repel = TRUE) 
```
...
We can see that Rank and Points are pointing in opposite directions along the x axis (first principal component).  
This makes sense because the more points you get, the higher ranked you will be.

We see that 100m and 100m hurdle correlates with rank. This means that being good at these events is more likely to give you a good rank. 
We see that Long Jump and High Jump are correlated with Points. This means that being good at these events is more likely to give you a high number of points.  

Bear in mind that a good score for 100m is lower whereas a good score for long jump is higher! 
